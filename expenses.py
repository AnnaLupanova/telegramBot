...
def get_today_statistics(user_id: str) -> str:
    ...
    cursor.execute("select sum(amount)"
                   f"from expense where date(created)=date('now', 'localtime') and user_id = '{user_id}'")
    ...
    cursor.execute("select sum(amount) "
                   ...
                   f"from category where is_base_expense=true) and user_id = '{user_id}'")
    result = cursor.fetchone()
    base_today_expenses = result[0] if result[0] else 0
    return (f"Расходы сегодня:\n"
            f"всего — {all_today_expenses} руб.\n"
            f"базовые — {base_today_expenses} руб. из {_get_budget_limit()} руб. в мес.\n\n"
            f"За текущий месяц: /month")


def get_month_statistics(user_id: str) -> str:
    """Возвращает строкой статистику расходов за текущий месяц"""
    now = _get_now_datetime()
    first_day_of_month = f'{now.year:04d}-{now.month:02d}-01'
    cursor = db.get_cursor()
    cursor.execute(f"select sum(amount) "
                   f"from expense where date(created) >= '{first_day_of_month}' and user_id = '{user_id}'")
    result = cursor.fetchone()
    if not result[0]:
...
def delete_expense(row_id: int, user_id: str) -> None:
